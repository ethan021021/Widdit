...
static CGFloat const kPageControlHeight = 35 * 2;
...
static NSString * const kNotificationButtonText = @"Enable Notifications";
static NSString * const kLocationButtonText = @"Enable Location";
...
//    self.bodyLabel.text = body;
    ...
    self.norificationBtn = [UIButton new];
    
    [self.norificationBtn setTitleColor:[UIColor blackColor] forState: UIControlStateNormal];
    [self.norificationBtn setTitle:kNotificationButtonText forState:UIControlStateNormal];
    [self.norificationBtn addTarget:self action:@selector(handleNotificationButtonPressed) forControlEvents:UIControlEventTouchUpInside];
    self.norificationBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
    self.norificationBtn.titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
    self.norificationBtn.titleLabel.numberOfLines = 2;
    self.norificationBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
    self.norificationBtn.hidden = YES;
    
    self.locationBtn = [UIButton new];
    [self.locationBtn setTitleColor:[UIColor blackColor] forState: UIControlStateNormal];
    [self.locationBtn setTitle:kLocationButtonText forState:UIControlStateNormal];
    [self.locationBtn addTarget:self action:@selector(handleLocationButtonPressed) forControlEvents:UIControlEventTouchUpInside];
    self.locationBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
    self.locationBtn.titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
    self.locationBtn.titleLabel.numberOfLines = 2;
    self.locationBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
...
- (void)viewWillLayoutSubviews {
    ...
    CGFloat bodyYOrigin = 5;
    
    self.pageVC.view.frame = self.view.frame;
    self.moviePlayerController.view.frame = self.view.frame;
    self.skipButton.frame = CGRectMake(CGRectGetMaxX(self.view.frame) - kSkipButtonWidth, CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kSkipButtonHeight - 10, kSkipButtonWidth, kSkipButtonHeight);
    self.norificationBtn.frame = CGRectMake(CGRectGetMaxX(self.view.frame) - kSkipButtonWidth, CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kSkipButtonHeight - 10, kSkipButtonWidth, kSkipButtonHeight);
    self.locationBtn.frame = CGRectMake(CGRectGetMaxX(self.view.frame) - kSkipButtonWidth, CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kSkipButtonHeight - 10, kSkipButtonWidth, kSkipButtonHeight);
...
//    self.bodyLabel.frame = CGRectMake(xPadding, bodyYOrigin, contentWidth, CGRectGetHeight(self.bodyLabel.frame));
    self.bodyLabel.frame = CGRectMake(xPadding, bodyYOrigin, contentWidth, 100);

}

- (void)makeSkipButtonRight {
    self.skipButton.frame = CGRectMake(CGRectGetMaxX(self.view.frame) - kSkipButtonWidth, CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kSkipButtonHeight - 10, kSkipButtonWidth, kSkipButtonHeight);
}

- (void)makeSkipButtonLeft {
    self.skipButton.frame = CGRectMake(0, CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kSkipButtonHeight - 10, kSkipButtonWidth, kSkipButtonHeight);
}
...
- (void)generateView {
    ...
    [self.view addSubview:self.norificationBtn];
...
- (void)handleNotificationButtonPressed {
    if (self.notificationHandler) {
        self.notificationHandler();
    }
}

- (void)handleLocationButtonPressed {

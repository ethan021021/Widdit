...
@IBDesignable public class ActiveLabel: UILabel {
    ...
    public class func parseText(text: String) -> [String] {
        var hashtagFilterPredicate: ((String) -> Bool)?
        let hashtagElements = ActiveBuilder.createElements(.Hashtag, from: text, range: NSRange(location: 0, length: text.utf16.count), filterPredicate: hashtagFilterPredicate)
        var hashtags:[String] = []
        for hashtag in hashtagElements {
            print(hashtag.element)
            switch hashtag.element {
            case .Hashtag(let tag):
                hashtags.append(tag.lowercaseString)
            default:
                print()
            }
        }
